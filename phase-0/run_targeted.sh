...
sudo -E ncu --metrics dram__bytes.sum,dram__bytes.sum.per_second,dram__throughput.avg.pct_of_peak_sustained_elapsed,sm__throughput.avg.pct_of_peak_sustained_elapsed,gpu__time_duration.sum \
  --kernel-name regex:mul_mat_vec_q \
  --launch-skip 500 --launch-count 10 \
  -o baseline_bandwidth_ncu4 --force-overwrite \
