...
sudo -E ncu --metrics l1tex__data_bank_conflicts_pipe_lsu.sum,smsp__sass_average_data_bytes_per_sector_mem_global_op_ld.pct,l1tex__t_sectors_pipe_lsu_mem_global_op_ld.sum \
  --kernel-name regex:mul_mat_vec_q \
  --launch-skip 500 --launch-count 5 \
  -o mem_efficiency_check --force-overwrite \
...
ls -la mem_efficiency_check.ncu-rep*
